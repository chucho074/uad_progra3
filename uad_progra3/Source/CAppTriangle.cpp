...
CAppTriangle::CAppTriangle() :
	CAppTriangle(CGameWindow::DEFAULT_WINDOW_WIDTH, CGameWindow::DEFAULT_WINDOW_HEIGHT)
{
}

/* */
CAppTriangle::CAppTriangle(int window_width, int window_height) :
	CApp(window_width, window_height),
	m_triangleShaderProgramID{ 0 },
	m_triangleVertexArrayObject{ 0 },
	m_triangleTextureID{ 0 },
	m_numFaces{ 0 }
{
	cout << "Constructor: CAppTriangle(int window_width, int window_height)" << endl;
...
CAppTriangle::~CAppTriangle()
{
	cout << "Destructor: ~CAppTriangle()" << endl;
	...
	if (m_triangleTextureID > 0 && getOpenGLRenderer() != nullptr)
	{
		getOpenGLRenderer()->deleteTexture(&m_triangleTextureID);
	}

	// Free graphics memory used for the texture
	if (m_triangleVertexArrayObject > 0 && getOpenGLRenderer() != nullptr)
	{
		getOpenGLRenderer()->freeGraphicsMemoryForObject(&m_triangleVertexArrayObject);
	}
}

/* */
void CAppTriangle::initialize()
{
	int    idx = 0;
	float  triangleSize = 2.0f;

	m_numFaces = 4 * 2; // 4 quads * 2 triangles per quad

	// SHADER
	// ----------------------------------------------------------------------------------------------------------------------------

	// Get shader for textured objects
	m_triangleShaderProgramID = getOpenGLRenderer()->getShaderProgramID(SHADER_PROGRAM_TEXTURED_OBJECT);

	if (m_triangleShaderProgramID == 0)
	{
	...
	int    numVertices = 4;
	int    numVertexData = numVertices * 3;          // 4 vertices * 3 components (x, y, z) per vertex
	...
	vertexData[idx++] = -triangleSize / 2.0f; vertexData[idx++] = triangleSize; vertexData[idx++] = triangleSize / 2.0f; // FRONT TOP LEFT
	vertexData[idx++] = triangleSize / 2.0f; vertexData[idx++] = triangleSize; vertexData[idx++] = triangleSize / 2.0f; // FRONT TOP RIGHT
	vertexData[idx++] = -triangleSize / 2.0f; vertexData[idx++] = 0.0f;     vertexData[idx++] = triangleSize / 2.0f; // FRONT BOTTOM LEFT
	vertexData[idx++] = triangleSize / 2.0f; vertexData[idx++] = 0.0f;     vertexData[idx++] = triangleSize / 2.0f; // FRONT BOTTOM RIGHT

	//vertexData[idx++] = -triangleSize / 2.0f; vertexData[idx++] = triangleSize; vertexData[idx++] = -triangleSize / 2.0f; // BACK TOP LEFT
	//vertexData[idx++] = triangleSize / 2.0f; vertexData[idx++] = triangleSize; vertexData[idx++] = -triangleSize / 2.0f; // BACK TOP RIGHT
	//vertexData[idx++] = -triangleSize / 2.0f; vertexData[idx++] = 0.0f;     vertexData[idx++] = -triangleSize / 2.0f; // BACK BOTTOM LEFT
	//vertexData[idx++] = triangleSize / 2.0f; vertexData[idx++] = 0.0f;     vertexData[idx++] = -triangleSize / 2.0f; // BACK BOTTOM RIGHT
	...
	faceIndices[idx++] = 0; faceIndices[idx++] = 1; faceIndices[idx++] = 3; // RIGHT #1
	//faceIndices[idx++] = 5; faceIndices[idx++] = 3; faceIndices[idx++] = 7; // RIGHT #2

	faceIndices[idx++] = 0; faceIndices[idx++] = 3; faceIndices[idx++] = 2; // LEFT #1
	...
	faceIndices[idx++] = 2; faceIndices[idx++] = 3; faceIndices[idx++] = 1; // BOTTOM #1
	...
	int    numNormals = 4;
	int    numNormalData = numNormals * 3;        // 4 different normals * 3 components (x, y, z) each. 
	...
	faceNormalIndices[idx++] = 3; faceNormalIndices[idx++] = 3; faceNormalIndices[idx++] = 3; // BOTTOM #1
...
//	int    numVertexUVData = numVertexUVs * 2;        // 8 vertices * 2 components (u,v) each vertex
	...
	int    numVertexUVs = 4;                         // 14 vertices (8 vertices) + 6 duplicated vertices
	int    numVertexUVData = numVertexUVs * 2;                     // 14 vertices * 2 components (u,v) each vertex
	...
	*/
	...
	faceUVIndices[idx++] = 0;   faceUVIndices[idx++] = 1;  faceUVIndices[idx++] = 3;  // RIGHT #1
	//faceUVIndices[idx++] = 10;  faceUVIndices[idx++] = 3;  faceUVIndices[idx++] = 11;  // RIGHT #2

	faceUVIndices[idx++] = 0;   faceUVIndices[idx++] = 3;  faceUVIndices[idx++] = 0;   // LEFT #1
	//faceUVIndices[idx++] = 0;   faceUVIndices[idx++] = 9;  faceUVIndices[idx++] = 2;   // LEFT #2

	//faceUVIndices[idx++] = 4;   faceUVIndices[idx++] = 0;  faceUVIndices[idx++] = 5;   // TOP #1
	//faceUVIndices[idx++] = 5;   faceUVIndices[idx++] = 0;  faceUVIndices[idx++] = 1;   // TOP #2

	faceUVIndices[idx++] = 2;   faceUVIndices[idx++] = 3; faceUVIndices[idx++] = 1;    // BOTTOM #1
	//faceUVIndices[idx++] = 3;   faceUVIndices[idx++] = 6; faceUVIndices[idx++] = 7;    // BOTTOM #2
	...
	bool loadedToGraphicsCard = getOpenGLRenderer()->allocateGraphicsMemoryForObject(
		&m_triangleShaderProgramID,   // Shader to use for this object
		&m_triangleVertexArrayObject, // NEW ID that is going to be created when saving this geometry to graphics memory
		vertexData,               // Vertices
	...
	if (!loadedToGraphicsCard)
	{
		...
		if (m_triangleVertexArrayObject > 0)
		{
			getOpenGLRenderer()->freeGraphicsMemoryForObject(&m_triangleVertexArrayObject);
			m_triangleVertexArrayObject = 0;
		}
	...
	else
	{
		...
		if (CWideStringHelper::GetResourceFullPath(MC_GRASS_3_TEXTURE, wresourceFilenameTexture, resourceFilenameTexture))
		{
			// Load texture
			m_triangleTextureID = 0;
			if (!CTextureLoader::loadTexture(resourceFilenameTexture.c_str(), &m_triangleTextureID, getOpenGLRenderer()))
			{
...
void CAppTriangle::run()
{
	...
	if (canRun())
	{
		...
		if (getGameWindow()->create(CAPP_PROGRA3_TRIANGLE))
		{
			initialize();
...
/* */
void CAppTriangle::update(double deltaTime)
{
...
void CAppTriangle::render()
{
	...
	{
		if (m_numFaces > 0 && m_triangleVertexArrayObject > 0 && m_triangleShaderProgramID > 0)
		{
			// White 
			...
			getOpenGLRenderer()->renderObject(
				&m_triangleShaderProgramID,                      // Shader to use to render the object
				&m_triangleVertexArrayObject,                    // ID for the geometry
				&m_triangleTextureID,                            // ID for the texture
...
/* */
void CAppTriangle::onMouseMove(float deltaX, float deltaY)
{
...
/* */
void CAppTriangle::executeMenuAction()
{
	if (getMenu() != NULL)
	{
