...
/* */
CAppFbxLoader::CAppFbxLoader() :
	CAppFbxLoader(CGameWindow::DEFAULT_WINDOW_WIDTH, CGameWindow::DEFAULT_WINDOW_HEIGHT) // C++11 ability to call one constructor from another
{
	cout << "Constructor: CAppFbxLoader()" << endl;
}

/* */
CAppFbxLoader::CAppFbxLoader(int window_width, int window_height) :
...
/* */
CAppFbxLoader::~CAppFbxLoader() {
	cout << "Destructor: ~CAppFbxLoader()" << endl;
	unloadCurrent3DModel();
}

/* */
void CAppFbxLoader::initialize() {
...
/* */
bool CAppFbxLoader::initializeMenu() {

	cout << "CAppFbxLoader::initializeMenu()" << endl;
	...
	if (getMenu() != nullptr)
	{
		...
		unsigned int textureFbxectId = 0;
		...
		if (CTextureLoader::loadTexture(resourceFilenameTexture.c_str(), &textureFbxectId, getOpenGLRenderer()))
		{
			// Set the generated texture object in the menu object
			menu->setTextureObjectId(textureFbxectId);
		}
		else
		{
			return false;
...
/* */
void CAppFbxLoader::update(double deltaTime)
{
...
/* */
bool CAppFbxLoader::load3DModel(const char * const filename) {

	unsigned int modelVertexArrayFbxect = 0;
...
/* */
void CAppFbxLoader::unloadCurrent3DModel()
{
	if (m_p3DModel != nullptr)
	{
...
/* */
void CAppFbxLoader::onF2(int mods) {
	...
	ofn.hwndOwner = NULL;
	ofn.lpstrFilter = L" Fbx Files\0*.fbx\0 Stl Files\0*.stl\0 3DS Files\0*.3ds\0 All files\0*.*\0";
...
/* */
void CAppFbxLoader::onF3(int mods)
{
...
/* */
void CAppFbxLoader::onMouseMove(float deltaX, float deltaY)
{
...
/* */
void CAppFbxLoader::moveCamera(float direction)
{
...
/* */
void CAppFbxLoader::executeMenuAction()
{
	if (getMenu() != nullptr)
	{
